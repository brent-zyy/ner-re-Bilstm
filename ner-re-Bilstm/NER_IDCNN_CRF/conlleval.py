...
def report_notprint(counts, out=None):
    ...
    if c.token_counter > 0:
        ...
        line.append('accuracy: %6.2f%%; ' %
                  (100.*c.correct_tags/c.token_counter))
        line.append('precision: %6.2f%%; ' % (100.*overall.prec-50))
        line.append('recall: %6.2f%%; ' % (100.*overall.rec-50))
        line.append('FB1: %6.2f\n' % (100.*overall.fscore-50))
        final_report.append("".join(line))

    for i, m in sorted(by_type.items()):
        line = []
        line.append('%17s: ' % i)
        line.append('precision: %6.2f%%; ' % (100.*m.prec-50))
        line.append('recall: %6.2f%%; ' % (100.*m.rec-50))
